...
const SUPPORTED_MCP_PROTOCOL_VERSIONS = new Set(
  SUPPORTED_MCP_PROTOCOL_VERSION_LIST,
);
...
const MCP_SERVER_INFO = {
  ...
  version: "1.1.0",
  description: "Query and prune user device event records.",
};
...
const QUERY_EVENTS_TOOL = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      type: {
        type: "string",
        description: "Event type filter (dot-separated lowercase alphanumeric).",
      },
      value: {
        type: "string",
...
const LIST_EVENT_TYPES_TOOL = {
  ...
  inputSchema: {
    ...
    properties: {
      hours: {
        type: "number",
        description: "Only list types seen in the last N hours. Omit to list every type ever recorded.",
        minimum: 0.001,
      },
    },
    required: [],
...
const DELETE_EVENTS_TOOL = {
  name: "delete_events",
  title: "Delete Events",
  description: "Permanently delete event records matching a time range, type, and/or value. Always run once without confirm to preview first.",
  inputSchema: {
    type: "object",
    additionalProperties: false,
    properties: {
      ...
      date: {
        type: "string",
        description: "Delete one local calendar day, written as YYYY-MM-DD.",
      },
      type: {
        type: "string",
        description: "Event type filter (dot-separated lowercase alphanumeric).",
      ...
      before_days: {
        type: "number",
        description: "Delete records older than N days from now.",
        minimum: 0.001,
      },
      confirm: {
        type: "boolean",
        description: "Must be true to actually delete. Omit to preview only.",
      },
    },
    required: [],
  },
...
async function callListEventTypesTool(
  args: Record<string, unknown>,
  sql: postgres.Sql,
): Promise<ToolCallResult> {
  try {
...
async function callDeleteEventsTool(
  ...
  try {
    ...
    if (!confirm) {
      ...
      return {
        content: [{ type: "text", text: `Preview: ${matched} records match filter ${description}. Re-run with confirm=true to delete.` }],
    ...
    return {
      content: [{ type: "text", text: `Deleted ${deleted.matched} records matching ${description}.` }],
...
async function callBarkNotifyTool(
  args: Record<string, unknown>,
): Promise<ToolCallResult> {
  const barkKey = process.env.BARK_KEY;
  if (!barkKey) {
    return {
      content: [{ type: "text", text: "Bark notification failed: BARK_KEY not configured." }],
      structuredContent: { success: false, message: "BARK_KEY not set" },
      isError: true,
    };
  }
  ...
  if (typeof title !== "string" || !title.trim()) {
    return {
      content: [{ type: "text", text: "Bark notification failed: title is required." }],
      structuredContent: { success: false, message: "title required" },
      isError: true,
    };
  }
  if (typeof body !== "string" || !body.trim()) {
    return {
      content: [{ type: "text", text: "Bark notification failed: body is required." }],
      structuredContent: { success: false, message: "body required" },
      isError: true,
    };
  }
  try {
    ...
    return {
      content: [{ type: "text", text: `Bark notification sent: ${title} - ${body}` }],
      structuredContent: { success: true },
      isError: false,
    };
  } catch (error) {
    return {
      content: [{ type: "text", text: "Bark notification failed: " + String(error) }],
      structuredContent: { success: false, message: String(error) },
      isError: true,
    };
  }
}
...
export async function handleMcpPost(
  ...
  if (Array.isArray(body)) {
    ...
    for (const item of body) {
      ...
      responses.push(await handleMcpRequest(message, sql, offsetMinutes, protocolVersion));
    }
    if (!responses.length) {
